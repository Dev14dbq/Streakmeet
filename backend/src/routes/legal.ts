...
router.get('/status/me', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
...
router.post('/accept', requireAuth, asyncHandler(async (req: AuthRequest, res: Response) => {
  const accepted = await acceptCurrentLegalForUser(req.userId!)
  res.json({ ok: true, ...accepted })
}))

// GET /api/legal/terms | /api/legal/privacy
router.get('/:slug', asyncHandler(async (req, res: Response) => {
