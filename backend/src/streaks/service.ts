...
import { generousStreakTimezone } from './calendar.js'
import { REMOTE_SELFIE_TTL_MS, expireStaleRemoteSelfieRequests } from './remoteSelfie.js'
import {
  getLocalDateString,
  normalizeTimezone,
  pairWhere,
...
import { findActiveUserByNickname } from '../common/account.js'
import { ErrorCodes, ApiHttpError } from '../common/errors.js'
import { notifyStreakRemind } from '../notifications/push.js'
...
export async function listStreaks(userId: string) {
  const streaks = await prisma.streak.findMany({
    where: { ...streakForUserWhere(userId), active: true },
    include: {
...
export async function createStreak(userId: string, partnerId: string) {
  if (!partnerId.trim()) throw new ApiHttpError(400, ErrorCodes.MISSING_FIELD)
  ...
  if (existing) throw new ApiHttpError(400, ErrorCodes.STREAK_EXISTS)
...
export async function remindPartner(userId: string, partnerNickname: string) {
  ...
  if (!partner) throw new ApiHttpError(404, ErrorCodes.USER_NOT_FOUND)
  const streak = await prisma.streak.findFirst({
  ...
  if (!sender) throw new ApiHttpError(404, ErrorCodes.USER_NOT_FOUND)
  const today = getLocalDateString(normalizeTimezone(streak.timezone))
  if (streak.lastMetDate === today) throw new ApiHttpError(400, ErrorCodes.STREAK_ALREADY_MET_TODAY)
...
function streakDetailInclude(page: number, limit: number, userId: string) {
  return {
    ...
    streakDays: {
      include: {
        meetProofs: { include: { uploadedBy: { select: { id: true, nickname: true } } } },
      },
...
export async function getStreakDetail(userId: string, param: string, page: number, limit: number) {
  ...
  if (isLegacyId) {
    ...
    if (!partner) throw new ApiHttpError(404, ErrorCodes.STREAK_NOT_FOUND)
  ...
  if (!streakId) throw new ApiHttpError(404, ErrorCodes.STREAK_NOT_FOUND)
  await expireStaleRemoteSelfieRequests(streakId)
  const streak = await prisma.streak.findUnique({ where: { id: streakId }, include })
  if (!streak || (streak.userAId !== userId && streak.userBId !== userId)) {
    throw new ApiHttpError(404, ErrorCodes.STREAK_NOT_FOUND)
  }
  return streak
